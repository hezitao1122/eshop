...
public class EnvController extends BaseController {
    ...
    public WebResp<EnvDto> findEnvDetail(@RequestParam("id") String id) {
        Optional<Env> optional = envRepo.findById(id);
    ...
    @Secured("ROLE_ADMIN")
    public WebResp<String> deleteEnv(@RequestParam("id") String id) {
        Optional<Env> optional = envRepo.findById(id);
        Env env = optional.get();
    ...
    public WebResp<String> updateEnv(@RequestBody EnvDto env) {
        Optional<Env> optional = envRepo.findById(env.getId());
        Env u = optional.get();
