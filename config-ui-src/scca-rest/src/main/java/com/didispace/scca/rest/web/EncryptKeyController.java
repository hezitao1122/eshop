...
public class EncryptKeyController extends BaseController {
    ...
    public WebResp<String> deleteEncryptKey(@RequestParam("encryptKeyId") String encryptKeyId) {
        EncryptKey encryptKey = new EncryptKey();
        encryptKey.setId(encryptKeyId);
        Example<EncryptKey> example1 = Example.of(encryptKey);
        Optional<EncryptKey> optional1 = encryptKeyRepo.findOne(example1);
        encryptKey = optional1.get();

        log.info("delete EncryptKey : " + encryptKey);
        encryptKeyRepo.delete(encryptKey);
    ...
    public WebResp<String> updateEncryptKey(@RequestBody EncryptKeyDto encryptKey) {
        EncryptKey updateEncryptKey = new EncryptKey();
        updateEncryptKey.setId(encryptKey.getId());
        Example<EncryptKey> example1 = Example.of(updateEncryptKey);
        Optional<EncryptKey> optional1 = encryptKeyRepo.findOne(example1);
        updateEncryptKey = optional1.get();
    ...
    public WebResp<String> deleteBatch(@RequestBody List<String> ids) {
