...
import com.alibaba.csp.sentinel.dashboard.datasource.entity.page.PageEntity;
import com.alibaba.csp.sentinel.dashboard.datasource.entity.page.PageResultEntity;
...
public class SystemController {
    ...
    @Autowired
    @Qualifier("systemRuleRedisProvider")
    private DynamicRuleProvider<List<SystemRuleEntity>> ruleProvider;
    @Autowired
    @Qualifier("systemRuleRedisPublisher")
...
//            List<SystemRuleEntity> rules = sentinelApiClient.fetchSystemRuleOfMachine(app, ip, port);
            ...
            List<SystemRuleEntity> lis = new ArrayList<>();
            if (rules != null && !rules.isEmpty()) {
                for (SystemRuleEntity entity : rules) {
                    if (entity.getIp().equals(ip) && entity.getPort().equals(port))
                        lis.add(entity);
                }
            }
            lis.forEach(sRule -> {
                loadTypeAndValue(sRule);
            });
            rules = repository.saveAll(rules);
            return Result.ofSuccess(lis);
    ...
    @PostMapping("/rules.json")
    public Result<List<SystemRuleEntity>> apiQueryMachineRules(@RequestBody PageEntity en) {
...
//            return Result.ofSuccess(lis);
            return PageResultEntity.ofSuccess(lis,en);
        } catch (Throwable throwable) {
            logger.error("Query machine system rules error", throwable);
            return Result.ofThrowable(-1, throwable);
        }
    ...
    public Result<SystemRuleEntity> apiAdd(@RequestBody SystemRuleEntity en) {
        if(en.getType() == null || en.getValue() == null || (en.getType()<0 || en.getType() > 4)){
            return Result.ofFail(-1, "type or value not null or type\\value error!");
        }
        loadValue(en);
        String app = en.getApp();
        String ip = en.getIp();
        Integer port = en.getPort();
        Double highestSystemLoad = en.getHighestSystemLoad();
        Double highestCpuUsage = en.getHighestCpuUsage();
        Long avgRt = en.getAvgRt();
        Long maxThread = en.getMaxThread();
        Double qps = en.getQps();
        Result<SystemRuleEntity> checkResult = checkBasicParams(app, ip, port);
        if (checkResult != null) {
            return checkResult;
        }
        ...
        loadTypeAndValue(entity);
        return Result.ofSuccess(entity);
    }


    @PostMapping("/save.json")
    public Result<SystemRuleEntity> apiUpdateIfNotNull(@RequestBody SystemRuleEntity en) {
        if(en.getId() == null){
            return Result.ofFail(-1, "id is not null !");
        }
        if(en.getType() == null || en.getValue() == null || (en.getType()<0 || en.getType() > 4)){
            return Result.ofFail(-1, "type or value not null or type\\value error!");
        }
        ...
        entity.setAvgRt(null);
        entity.setHighestSystemLoad(null);
        entity.setMaxThread(null);
        entity.setQps(null);
        entity.setHighestCpuUsage(null);
    ...
    public Result<?> delete(@RequestBody SystemRuleEntity en) {
        Long id = en.getId();
        if (id == null) {
            return Result.ofFail(-1, "id can't be null");
        }
    ...
    private void loadValue(SystemRuleEntity entity){
        /**
         * 类型 0-LOAD 1-RT 2-线程数 3-入口QPS 4-CPU使用率
         */
        ...
        switch (type){
            case 0:
                entity.setHighestSystemLoad(value);

                break;
            case 1:
                entity.setAvgRt(value.longValue());
                break;
            case 2:
                entity.setMaxThread(value.longValue());
                break;
            case 3:
                entity.setQps(value);
                break;
            case 4:
                entity.setHighestCpuUsage(value);
                break;
        }
    }

    private void loadTypeAndValue(SystemRuleEntity entity){
        Double load = entity.getHighestSystemLoad();
        Long rt = entity.getAvgRt();
        Long maxThread = entity.getMaxThread();
        Double qps = entity.getQps();
        Double cpu = entity.getHighestCpuUsage();

        if(load != null && load != -1.0D){
            entity.setType(0);
            entity.setValue(load);
        }else if(rt != null && rt != -1L){
            entity.setType(1);
            entity.setValue(rt.doubleValue());
        }else if(maxThread != null && maxThread != -1L){
            entity.setType(2);
            entity.setValue(maxThread.doubleValue());
        }else if(qps != null && qps != -1.0D){
            entity.setType(3);
            entity.setValue(qps);
        }else if(cpu != null && cpu != -1.0D){
            entity.setType(4);
            entity.setValue(cpu);
        }
