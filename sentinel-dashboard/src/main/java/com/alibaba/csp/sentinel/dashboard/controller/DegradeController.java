...
//            List<DegradeRuleEntity> rules = sentinelApiClient.fetchDegradeRuleOfMachine(app, ip, port);
            ...
            List<DegradeRuleEntity> lis = new ArrayList<>();
            rules.forEach(r ->{
                if(r.getIp().equals(ip) && r.getPort().equals(port)){
                    lis.add(r);
                }
            });
    ...
    public Result<List<DegradeRuleEntity>> queryMachineRules( @RequestBody PageEntity entity) {
...
//            List<DegradeRuleEntity> rules = sentinelApiClient.fetchDegradeRuleOfMachine(app, ip, port);
            ...
            if (rules != null && !rules.isEmpty()) {
                for (DegradeRuleEntity entity1 : rules) {
                    if(entity1.getIp().equals(ip)&& entity1.getPort().equals(port))
                        lis.add(entity1);
                }
            }
            rules = repository.saveAll(rules);
//            return Result.ofSuccess(lis);
            return PageResultEntity.ofSuccess(rules,entity);
        } catch (Throwable throwable) {
            logger.error("queryApps error:", throwable);
            return Result.ofThrowable(-1, throwable);
        }
    ...
    public Result<DegradeRuleEntity> add(HttpServletRequest request, @RequestBody DegradeRuleEntity entity) {
        ...
        String ip = entity.getIp();
        Integer port = entity.getPort();
        String limitApp = entity.getLimitApp();
        String resource = entity.getResource();
        Double count = entity.getCount();
        Integer timeWindow = entity.getTimeWindow();
