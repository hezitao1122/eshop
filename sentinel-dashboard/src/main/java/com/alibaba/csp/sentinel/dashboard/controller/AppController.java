...
public class AppController {
    ...
    @PostMapping(value = "/machine/machines.json")
    public Result<List<MachineInfoVo>> getMachinesByApp(@RequestBody MachineInfo info) {
        if (info == null ||  info.getApp()== null || info.getApp().equals("")) {
            return Result.ofFail(500,"APP is not null !");
        }
        AppInfo appInfo = appManagement.getDetailApp(info.getApp());
        if (appInfo == null) {
    ...
    public Result<List<MachineInfoVo>> getMachinesByApp1(@PathVariable String app) {
    ...
    @PostMapping(value = "/machine/remove.json")
    public Result<String> removeMachineById(@RequestBody PageEntity en) {
